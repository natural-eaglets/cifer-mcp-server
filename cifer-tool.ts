...
function shouldProvisionDatabase(args: string[], env: Record<string, string>): boolean {
  if (args.includes("--no-provision-db")) return false;
  if (args.includes("--provision-db")) return true;
  return !!env.CIFER_DB_PROVISIONER_URL && !env.DATABASE_URL;
}
...
function writeEnvFile(next: Record<string, string>) {
  const knownOrder = [
    ...
    "CIFER_RPC_URL",
    "CIFER_CHAIN_ID",
    "CIFER_DB_PROVISIONER_URL",
    "CIFER_DB_INSTALLATION_ID",
    "DATABASE_URL",
    "PGHOST",
    "PGPORT",
    "PGUSER",
...
async function cmdInit(args: string[]) {
  ...
  const wantsDbProvisioning = shouldProvisionDatabase(args, env);
  const provisionerUrl =
    parseFlag(args, "--provisioner-url") ??
    env.CIFER_DB_PROVISIONER_URL ??
    process.env.CIFER_DB_PROVISIONER_URL;
  const dbInstallationId =
    parseFlag(args, "--db-installation-id") ??
    env.CIFER_DB_INSTALLATION_ID ??
    address.toLowerCase();
  let dbProvisioning: ReturnType<typeof databaseEnvEntries> | null = null;
  let dbProvisioned = false;
  let dbProvisioningReused = false;

  if (wantsDbProvisioning) {
    if (!provisionerUrl) {
      fail(
        "Database provisioning was requested, but no provisioner URL was provided. Pass --provisioner-url <url> or set CIFER_DB_PROVISIONER_URL."
      );
      return;
    }
    ...
    dbProvisioning = databaseEnvEntries(provisioned, provisionerUrl);
    Object.assign(env, dbProvisioning);
    writeEnvFile(env);
    dbProvisioned = true;
    dbProvisioningReused = provisioned.reused;
  }
  ...
  if (dbProvisioning) {
    box([
      dbProvisioningReused
        ? "🗄️  Existing PostgreSQL credentials refreshed in .env."
        : "🗄️  PostgreSQL credentials saved to .env.",
      `   Database:     ${dbProvisioning.PGDATABASE}`,
      `   User:         ${dbProvisioning.PGUSER}`,
      `   Host:         ${dbProvisioning.PGHOST}:${dbProvisioning.PGPORT}`,
      "   Ready for     CREATE TABLE / INSERT / SELECT from your agent.",
    ]);
  }
  ...
  ok({
    ...
    databaseReused: dbProvisioningReused,
    databaseConfigured: !!env.DATABASE_URL,
    databaseName: env.PGDATABASE ?? null,
    verified,
    authorized,
    isSyncing,
...
Commands:
  init [--secret-id <N>] [--force]     Generate agent wallet + write .env (start here)
       [--provision-db] [--provisioner-url <url>] [--db-installation-id <id>]
...
For automatic PostgreSQL bootstrap, point init at a provisioner:
  cifer-mcp init --provision-db --provisioner-url https://...
`);
  process.exit(0);
}
